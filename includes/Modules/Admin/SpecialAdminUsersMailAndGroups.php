<?php
/**
 * Page spéciale temporaire pour la gestion des comptes utilisateurs
 */
...
class SpecialAdminUsersMailAndGroups extends \SpecialPage {
	/**
	...
	public function __construct() {
		parent::__construct( 'MgwAdminUsers', 'editinterface' );
	}
	...
	public function execute( $par ) {
		...
		if (
			$request->wasPosted() &&
			$request->getCheck( 'saveuseremail' ) &&
			$this->mTarget !== null &&
			$user->matchEditToken( $request->getVal( 'wpEditToken' ), $this->mTarget )
		) {
			$targetUser = \User::newFromName($this->mTarget);
			$targetUser->setEmail( $request->getVal( 'new-email' ) );
			$targetUser->saveSettings();
			$out->addScript(
				'<script LANGUAGE=\'JavaScript\'>
					window.alert("Le mail de l\'utilisateur ' . $this->mTarget . ' a bien été modifié");
					window.location.href="/wiki/index.php?title=Spécial%3AMgwAdminUsers&user=' . $this->mTarget . '";
				</script>'
			);
		}
		...
		if (
			$request->wasPosted() &&
			$request->getCheck( 'resetuserpassword' ) &&
			$this->mTarget !== null &&
			$user->matchEditToken( $request->getVal( 'wpEditToken' ), $this->mTarget )
		) {
			$targetUser = \User::newFromName($this->mTarget);
			$passwordReset = new \PasswordReset( $this->getConfig(), AuthManager::singleton() );
			$statusValue = $passwordReset->execute( $user, $this->mTarget, $targetUser->getEmail() );
			if ( !$statusValue->isOK() ){
				$mess = wfMessage($statusValue->getErrors()[0]['message'])->parse();
			}
			else {
				$mess = "Un nouveau mot de passe a été envoyé à l'utilisateur " . $this->mTarget . ".";
			}
			$mess = str_replace( '
', '', $mess );
			$href = '/wiki/index.php?title=Spécial%3AMgwAdminUsers&user=' . $this->mTarget;
			$out->addScript(
				"<script LANGUAGE='JavaScript'>
					window.alert(\"$mess\");
	...
	function editUserGroupsForm( $username ) {
		...
		$this->showEditUserEmail( $user );
		$this->showRestPassword( $user );
	...
	function switchForm() {
		...
		$this->getOutput()->addHTML(
			...
			Xml::submitButton(
				$this->msg( 'adminusers-showuser' )->text()
			) .
	...
	/**
	 * Show the form to edit user mail.
	 *
	 * @param User|UserRightsProxy $user User or UserRightsProxy you're editing
	 */
	protected function showEditUserEmail ($user) {
		if (in_array('sysop',$this->getUser()->getGroups())){
			...
			$this->getOutput()->addHTML(
				...
				Xml::element(
					'legend',
					[],
					$this->msg( 'adminusers-emailfield' )->text()
				)
			);
			$this->getOutput()->addHTML(
				'<em><strong>'.$email.'</strong></em>' .
				'<br>' .
				Xml::openElement( 'table', [ 'id' => 'mw-useremail-table-outer' ] ) .
					"<tr>
						<td class='mw-label'>" .
							Xml::label( $this->msg( 'adminusers-emailchange' )->text(), 'wpEmail' ) .
						"</td>
						<td class='mw-input'>" .
							Xml::input( 'new-email', 60, ''
								//$this->getRequest()->getVal( 'new-email', false )
								, [
					...
					<tr>
						<td></td>
						<td class='mw-submit'>" .
							Xml::submitButton( $this->msg( 'adminusers-saveusermail' )->text(),
								[ 'name' => 'saveuseremail' ]
							) .
						"</td>
					</tr>" .
	 ...
	 * @param User|UserRightsProxy $user User or UserRightsProxy you're editing
	 */
	protected function showRestPassword ($user) {
		if (in_array('sysop',$this->getUser()->getGroups())){
			$email = $user->getEmail();
			$this->getOutput()->addHTML(
				Xml::openElement(
				...
				Xml::element(
					'legend',
					[],
					$this->msg( 'adminusers-resetfield' )->text()
				)
			);
			$this->getOutput()->addHTML(
				Xml::openElement( 'table', [ 'id' => 'mw-useremail-table-outer' ] ) .
					"<tr>
						<td class='mw-input'>" .
							Xml::input( 'user-email', 60, $email, [
								'id' => 'user-email', 'type' => 'hidden',
								// HTML maxlength uses "UTF-16 code units", which means that characters outside BMP
								// (e.g. emojis) count for two each. This limit is overridden in JS to instead count
								// Unicode codepoints.
								'maxlength' => CommentStore::COMMENT_CHARACTER_LIMIT,
					...
					<tr>
						<td></td>
						<td class='mw-submit'>" .
							Xml::submitButton( $this->msg( 'adminusers-resetpass-submit' )->text(),
								[ 'name' => 'resetuserpassword' ]
							) .
						"</td>
					</tr>" .
				Xml::closeElement( 'table' ) . "\n" .
				'<p><small>' . $this->msg( 'adminusers-resetpass-notice' )->text() . '</p></small>' .
				Xml::closeElement( 'fieldset' )
			);
		}
	}
