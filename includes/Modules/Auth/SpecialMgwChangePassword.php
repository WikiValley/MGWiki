...
/**
 * Special to change password
 * adapté de SpecialChangeCredentials.php pour forcer l'édition du nouveau mot de passe:
 * - titre adapté pour qu'il soit plus compréhensible
 * - lien vers la page parente supprimé
 * - suppression du bouton "annuler" (=> procédure obligatoire)
 */
class SpecialMgwChangePassword extends \AuthManagerSpecialPage {
	...
	public function __construct( $name = 'MgwChangePassword' ) {
	...
	public function execute( $subPage ) {
		...
		$this->loadAuth( 'MediaWiki\Auth\PasswordAuthenticationRequest' );
    ...
    // ADAPTATION MGW :
    $form->addHeaderText( '<strong>' . $this->msg( 'mgw-changepassword-header', $info['account'] )->text() . '</strong>' );
		$form->setSubmitTextMsg( "mgw-changepassword-submit" );
